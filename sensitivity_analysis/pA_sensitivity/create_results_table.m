...
for p_i = 1:size(pAs, 2)
    
    p_label = pAs{p_i}
    
    %path to the results directory:
    base_dirname = [pwd() '\R0_x_VE_scan_H14_tf_1M\' date_tag '\pA_' p_label '\']
    
    %output_filename = [base_dirname, 'output_table_R0xVE_scan_H14_tf400k.csv']
    
    output_filename = [base_dirname, 'output_t_relbase_R0xVE_pA_' p_label '.csv']
    ...
    for r_i = 1:numel(R0s)
        ...
        for v_i = 1:numel(VEs)
            ...
            run_tag = ['Run_R0_', R0_tag, '_pA_' p_label '_VE_' VE_tag];
