...
function main()
    ...
    include("./Quar_ABM_disease_no_R0_no_pasymp.jl")
    ...
    output_dirname = pwd() * "\\R0xVE_pA_corr_VE\\" * date *"\\"
    if !isdir(output_dirname)
     mkpath(output_dirname)
      ...
      # default
      #p_asymp =  pA # fraction of asymptomatic infections

      # tag = "pA_$p_asymp"
      # tag = replace(tag, "." => "-")
      # label = label*tag*"_"
      #
      # output_dirname = pwd() * "\\R0_x_VE_scan_H14_tf_1M\\" * date *"\\"*tag
      # #output_dirname = output_dirname * "\\" * tag
      # if !isdir(output_dirname)
      #   mkpath(output_dirname)
      # end
  ...
  for r_i in 1:size(R0s, 1)#, 0]#, 2]
    for e_i in 1:size(VEs, 1)
      ...
      #for t_i in [1]#, 2, 3]
            ...
            global p_asymp = 1.0 - (0.67 * (1.0 - VE))
            # symptomatic fraction reduced from baseline based on VE
