...
%rng(2)
%rng(3)
output_dirname = [pwd, '\output\Delta_calib_2021_08_12'];
...
print_sensitivity_timeline_flag = false;
print_TOST_flag = true;

plot_sensitivity_timeline_flag = false;
...
% smaller dispersion means more heterogeneity
dispersion = 0.1;
...
%             A3_diff_Test = A3_test - A3_2_test
    ...
    figure(2)
    h2=histogram(R0_n, 'normalization', 'probability', 'BinWidth', 1);
    ...
    if print_TOST_flag
        ...
        output_filename_4 = ['\SAR_hist_beta_' strrep(num2str(beta), '.', '-') '.csv'];
        writetable(table(SAR_hist_x, SAR_hist_y), [output_dirname, output_filename_4]);
        
        output_filename_5 = ['\SAR_distfit_beta_' strrep(num2str(beta), '.', '-') '.csv'];
        writetable(table(SAR_dist_fit_x, SAR_dist_fit_y), [output_dirname, output_filename_5]);
